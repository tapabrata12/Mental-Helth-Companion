...
"""
#####################################################################################################
This part is where we make the schema about the Report generation part by the LLM
#####################################################################################################
"""
class PHQ9ReportResult(BaseModel):
    model_config = ConfigDict(extra='forbid', str_strip_whitespace=True)
    insight_report: str = Field(..., description="A warm, plain-language explanation of what the PHQ-9 result may suggest, grounded in the provided source material.")
    remedies: list[str] = Field(default_factory=list, description="A list of 3 to 5 short, practical coping tips (e.g. breathing exercises, sleep hygiene, gentle self-care). This list must not be empty.")
    sources: list[str] = Field(default_factory=list, description="A list of just the document filenames used as source material, e.g. ['feeling-good-the-new-mood-therapy.pdf', 'mhGAP.pdf']. Do not include quotes or explanations here — filenames only.")